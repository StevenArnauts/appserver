...
namespace Core {
	...
	internal class ProcessApplicationHost : ApplicationHost, IDisposable {
		...
		internal override void Initialize(Context context, Type bootstrapper) {
			...
			this.Connection.Initialize(bootstrapper.AssemblyQualifiedName, initializeMethod);
		}
		...
		internal override void Start() {
			...
			this.Connection.Start(startMethod);
		...
		private Host.Server Connection {
			get {
				Host.Server host = (Host.Server)Activator.GetObject(typeof(Host.Server), this._url);
